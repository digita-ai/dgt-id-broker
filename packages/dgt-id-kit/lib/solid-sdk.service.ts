import { login, getSolidDataset, handleIncomingRedirect, getThing, getUrlAll, logout, getStringNoLocale, Thing } from '@digita-ai/inrupt-solid-client';
import { Session } from './models/session.model';
import { Profile } from './models/profile.model';
import { Issuer } from './models/issuer.model';
import { SolidService } from './solid.service';

// TODO: check and add https/http
...
export class SolidSDKService implements SolidService {
  ...
  async getSession(): Promise<Session> {
    ...
    return session && session.isLoggedIn && session.webId ? { webId: session.webId } : Promise.reject('No session with logged in webid found.');
  ...
  async login(webId: string): Promise<void> {
    ...
    if (!webId) {
      ...
      throw new Error(`WebId should be set.`);
    ...
    if (!issuer) {
      ...
      throw new Error(`Issuer should be set.`);
  ...
  async getProfile(webId: string): Promise<Profile> {
    ...
    return { uri: webId, name: name ?? undefined };
  ...
  private async profileThing(webId: string): Promise<Thing> {
    ...
    if (!webId) {
      ...
      throw new Error(`WebId must be set.`);

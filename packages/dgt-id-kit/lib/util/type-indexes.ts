...
export const createTypeIndexes = async (webId: string): Promise<{
 ...
 */
export const getInstanceForClass = async (webId: string, forClass: string): Promise<string> => {
 ...
 */
export const saveInstanceForClass = async (webId: string, forClass: string, location: string): Promise<string> => {
  ...
  const typeIndexUrl = getUrl(profile, 'http://www.w3.org/ns/solid/terms#publicTypeIndex') ?? (await createTypeIndexes(webId)).publicTypeIndex;

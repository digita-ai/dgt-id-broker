...
import { issuer, clientId, scope, webId, redirectUri, profileWithIssuers, mockedResponseValidSolidOidc, mockedResponseInvalidSolidOidc, issuer1, clientSecret, authorizationCode, codeChallenge, codeVerifier, handleAuthRequestUrl, getAuthorizationCode, state } from '../../test/test-data';
...
describe('loginWithIssuer()', () => {
  ...
  it('should call authRequest from the oidc module', async () => {
    ...
    expect(spy).toHaveBeenCalledWith(issuer, clientId, scope, redirectUri, codeChallenge, state, handleAuthRequestUrl);

  });

  const loginWithIssuerParams = { issuer, clientId, scope, redirectUri, codeChallenge };
...
describe('loginWithWebId()', () => {
  ...
  it('should call loginWithIssuer', async () => {
    ...
    expect(spy).toHaveBeenCalledWith(
      issuer1.url.toString(), clientId, scope, redirectUri, codeChallenge, state, handleAuthRequestUrl
    );

  });

  const loginWithWebIdParams = { webId, clientId, scope, redirectUri, codeChallenge };

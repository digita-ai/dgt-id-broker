...
export class PkceAuthRequestHandler extends HttpHandler {
  ...
  handle(context: HttpHandlerContext): Observable<HttpHandlerResponse> {
    ...
    if (!challenge) {
      ...
      return of(createErrorResponse(400, 'invalid_request', 'A code challenge must be provided.'));
    ...
    if (!method) {
      ...
      return of(createErrorResponse(400, 'invalid_request', 'A code challenge method must be provided'));

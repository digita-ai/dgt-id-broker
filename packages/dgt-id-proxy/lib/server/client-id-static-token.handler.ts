...
import { parseQuads, parseOidcRegistrationStatement, getWebID, retrieveAndValidateWebId } from '../util/process-webid';
...
export class ClientIdStaticTokenHandler extends HttpHandler {
  ...
  private checkWebId(clientId: string, grantType: string): Observable<Partial<OidcClientMetadata>> {
    ...
    return retrieveAndValidateWebId(clientId).pipe(

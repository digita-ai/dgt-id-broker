    ...
    return from(this.upstreamStateToClientStateStore.get(upstreamState)).pipe(
      switchMap((clientState) => {
        ...
        if (!clientState) {
          ...
          return throwError(() => new Error('No clientState was found in the upstreamStateToClientStateStore'));
      ...
      switchMap(([ clientState, clientRedirectUri ]) => {
        ...
        if (!clientRedirectUri) {
          ...
          return throwError(() => new Error(`The clientRedirectUri "${clientRedirectUri}"  was found in the clientStateToClientRedirectUriStore`));

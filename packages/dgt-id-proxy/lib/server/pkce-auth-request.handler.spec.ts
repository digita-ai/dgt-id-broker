...
describe('PkceAuthRequestHandler', () => {
  ...
  beforeEach(async () => {
    ...
    }, method: 'GET', path: '/auth?response_type=code&code_challenge=F2IIZNXwqJIJwWHtmf3K7Drh0VROhtIY-JTRYWHUYQQ&code_challenge_method=S256&scope=openid&client_id=http%3A%2F%2Flocalhost:3002%2Fjaspervandenberghen%2Fprofile%2Fcard%23me&redirect_uri=http%3A%2F%2Flocalhost:3001%2Frequests.html'
    , query: {
      'response_type': 'code', 'code_challenge': 'F2IIZNXwqJIJwWHtmf3K7Drh0VROhtIY-JTRYWHUYQQ',
      'code_challenge_method': 'S256', 'scope': 'openid', 'client_id': 'http%3A%2F%2Flocalhost:3002%2Fjaspervandenberghen%2Fprofile%2Fcard%23me',
  ...
  describe('handle', () => {
    ...
    it('should error when no context request query is provided', async () => {
      context.request.query = null;
      await expect(() => pkceHandler.handle(context).toPromise()).rejects.toThrow('No query was included in the request');
    ...
    it('should error when no code_challenge was provided', async () => {
      ...
      noChallengeContext.request.query.code_challenge = null;
      const response =  {
      ...
      noChallengeContext.request.query.code_challenge = undefined;
      await expect(pkceHandler.handle(noChallengeContext).toPromise()).resolves.toEqual(response);
    });

    it('should error when no code_challenge_method was provided', async () => {
      const noChallengeContext = context;
      noChallengeContext.request.query.code_challenge_method = null;
      const response =  {
      ...
      await expect(pkceHandler.handle(noChallengeContext).toPromise()).resolves.toEqual(response);
      noChallengeContext.request.query.code_challenge_method = undefined;
      await expect(pkceHandler.handle(noChallengeContext).toPromise()).resolves.toEqual(response);
    });

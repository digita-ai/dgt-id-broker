...
export const createVariables = (args: string[]): Record<string, any> => {
  ...
  const { argv: params } = yargs(hideBin(args))
    ...
    .options({
      ...
      proxyUri: { type: 'string', alias: 'u', describe: 'URL of the proxy', demandOption: true },
      upstreamUri: { type: 'string', alias: 'U', describe: 'URL of the upstream server', demandOption: true },
      mainModulePath: { type: 'string', alias: 'm' },
      openidConfigurationFilePath: { type: 'string', alias: 'o', describe: 'relative path to the OIDC configuration', demandOption: true },
      jwksFilePath: { type: 'string', alias: 'j', describe: 'relative path to the JWKs', demandOption: true },

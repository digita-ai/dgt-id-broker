...
generateCodeChallenge(code_verifier).then((code_challenge) => {
  ...
  window.location = `http://localhost:${env.VITE_OIDC_PORT}/${env.VITE_AUTH_ENDPOINT}?response_type=code&code_challenge=${code_challenge}&code_challenge_method=S256&scope=openid&client_id=http%3A%2F%2Flocalhost%3A${env.VITE_CSS_PORT}%2Fclientapp%2Fprofile&redirect_uri=http%3A%2F%2F${env.VITE_IP}:${env.VITE_PORT}%2Frequests.html`
})
